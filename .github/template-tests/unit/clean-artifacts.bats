#!/usr/bin/env bats
# Unit tests for scripts/gc/clean-artifacts.sh
...
@test "exits cleanly when gh not available" {
  ...
  run bash -c "cd '$TEST_REPO' && PATH='$no_gh_path' bash scripts/gc/clean-artifacts.sh"
  assert_success
...
@test "exits cleanly when GITHUB_TOKEN not set" {
  ...
  run bash -c "cd '$TEST_REPO' && bash scripts/gc/clean-artifacts.sh"
  assert_success
