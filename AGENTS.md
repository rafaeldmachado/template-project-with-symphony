...
4. **Update docs when you change behavior.** Docs are first-class — treat them as part of the deliverable, not an afterthought. If you add a feature, change a workflow, or modify a script, update the relevant docs (README.md, docs/, CLAUDE.md, AGENTS.md) in the same PR.
5. Do not modify `scripts/checks/`, `tests/structural/`, or CI workflows without approval.
6. Do not add dependencies without documenting the reason in the PR.
7. Use grep-friendly errors: `ERROR: [module] description` on single lines.
8. Keep files focused and under 300 lines.
9. Parse data shapes at boundaries — no YOLO probing of untyped data.
10. Prefer shared utilities over hand-rolled helpers for repeated patterns.
